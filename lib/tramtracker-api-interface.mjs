...
export class TramTrackerAPIInterface {
  ...
  /**
   * Parses a TramTracker date
   * 
   * @param {string} date A TramTracker Date in the format /Date(1719882240000+1000)/
   * @returns {Date} the processed date
   */
  static parseDateMS(date) {
    return parseInt(date.slice(0, -1).match(/(\d+)\+/)[1])
  }
}