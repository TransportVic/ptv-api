...
export class VLinePlatformService {
  ...
  static fromVLineData($, service, apiInterface) {
    ...
    let consistExists = !$('VConsistSubType', service).attr('i:nil')
    let consistSize = consistExists ? parseInt($('VConsistCount', service).text()) : -1
    let consistType = consistExists ? $('VConsistSubType', service).text() : null
