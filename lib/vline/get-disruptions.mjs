...
export class VLineDisruption {
  ...
  static fromPlannedData($, disruption) {
    ...
    let startTime = new Date($('VScheduledStartUtc', disruption).text() + 'Z')
    let endTime =  new Date($('VScheduledEndUtc', disruption).text() + 'Z')
  ...
  static fromLiveData($, disruption) {
    let title = $('VTitle', disruption).text()
    let details = $('VDescription', disruption).text()
