...
import { GetLiveDisruptionsDetailsAPI } from './get-live-disruptions.mjs'
...
export default class VLineInterface extends TransitInterface {
  ...
  * 
  * @returns {Promise<VLineLocations>} A list of V/Line locations
  */
 async getAllLocations() {
   ...
   * @returns {Promise<VLineDisruptions>} A list of planned V/Line disruptions
   */
 async getPlannedDisruptions(line, days, includeProposed) {
   ...
   * @returns {Promise<VLineJourneys>} A list of the next 5 journeys
   */
   ...
   * @returns {Promise<VLineStoppingPattern>} The stopping pattern data
   */
   ...
   * @returns {Promise<VLinePlatformServices>} A list of departures from the station
   */
  async getDepartures(location, direction, minutes) {
   ...
   * @returns {Promise<VLinePlatformServices>} A list of arrivals into the station
   */
  async getArrivals(location, direction, minutes) {
   ...
   * @returns {Promise<VLineDisruptions>} A list of planned V/Line disruptions
   */
  async getLiveDisruptionsDetails() {
    let disruptions = new GetLiveDisruptionsDetailsAPI()
    await disruptions.fetch(this.#api)
    return disruptions
  }
}