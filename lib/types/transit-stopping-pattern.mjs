...
export default class TransitStoppingPattern {
  ...
  getStops() {
    ...
    const liveTimes = rawStops.reduce((acc, stop) => {
      let stopCode = `${stop.stop_id}_${stop.scheduled_departure_utc}`
      if (!acc[stopCode]) acc[stopCode] = stop.estimated_departure_utc
    ...
    return rawStops.filter((stop, i) => {
      return i === 0 || !(
        rawStops[i - 1].stop_id === stop.stop_id &&
        rawStops[i - 1].scheduled_departure_utc === stop.scheduled_departure_utc
      )
    }).map(stop => {
      let stopID = stop.stop_id
      let stopData = stopsData[stopID]

      let stopCode = `${stopID}_${stop.scheduled_departure_utc}`
      if (stopsSeen.includes(stopCode)) return null
      stopsSeen.push(stopCode)

      if (!stop.estimated_departure_utc && liveTimes[stopCode]) {
        stop.estimated_departure_utc = liveTimes[stopCode]
      }
