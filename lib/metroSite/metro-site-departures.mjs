...
export class MetroSiteDepartureTrip {
  ...
  constructor(tdn, stops, date) {
    this.tdn = tdn
    this.direction = parseInt(tdn[3]) % 2 === 0 ? 'Up' : 'Down'
  ...
  createStops(stops) {
    const stopObjects = stops.map(stop => {
    ...
    }).sort((a, b) => a.scheduledDepartureMinutes - b.scheduledDepartureMinutes)

    const thlIndex = stopObjects.findIndex(stop => stop.stationName === 'Town Hall')
    if (thlIndex !== -1) {
      if (this.direction === 'Up') return stopObjects.slice(0, thlIndex + 1)
      else return stopObjects.slice(thlIndex)
    }
    return stopObjects
  }
