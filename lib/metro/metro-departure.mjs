...
import cityStations from './metro-data/city-stations.json' with { type: 'json' }
...
export default class MetroDeparture extends TransitDeparture {
  ...
  isFormingTrainInLoop() {
    ...
    if (this.#departureStop === 'Southern Cross' && (lineGroup === 'crossCity' || lineGroup === 'northern')) {
  ...
  static fromPTVData(apiInterface, departure, route, run, direction, stops) {
    ...
    let metroDirection = MetroUtils.createDirection(metroLine, direction.direction_id, direction.direction_name, run.destination_name)
