...
export default class MetroStoppingPattern extends TransitStoppingPattern {
 ...
 getRoute() {
  ...
  #getOrigin() {
    ...
    let thl = this.stops.find(stop => stop.stationName === 'Town Hall')
    if (thl) return thl.stationName
  ...
  trimStops() {
    ...
    let startIndex = 0
    let endIndex = this.stops.length - 1

    let fssIndex = this.stops.findIndex(stop => stop.stationName === 'Flinders Street')
    let thlIndex = this.stops.findIndex(stop => stop.stationName === 'Town Hall')

    if (this.runData.direction.railDirection === 'Up') {
      if (fssIndex !== -1) endIndex = fssIndex
      else if (thlIndex !== -1) endIndex = thlIndex
    } else if (this.runData.direction.railDirection === 'Down') {
      let lastFSSIndex = this.stops.findLastIndex(stop => stop.stationName === 'Flinders Street')
      let lastTHLIndex = this.stops.findLastIndex(stop => stop.stationName === 'Town Hall')

      if (fssIndex !== -1) startIndex = lastFSSIndex
      else if (thlIndex !== -1) startIndex = lastTHLIndex
    }
    ...
    NOTE: SSS may not exist on night network, only FSS gets duplicated
    */
    if (CROSS_CITY.includes(this.routeData.routeName)) {
      startIndex = this.stops
        .map((stop, i) => ({ stop, i }))
        .findLast(({stop}) => stop.stationName === origin)
        .i
    }
    ...
    this.runData.destination = this.stops[this.stops.length - 1].stationName
  }
