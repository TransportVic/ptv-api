...
import { dateToGTFSDate, getDateNow, parseISOTime } from '../date-utils.mjs'
...
export default class MetroStoppingPattern extends TransitStoppingPattern {
 ...
 getRoute() {
  ...
  trimStops() {
    ...
    */
    if (this.stops[startIndex + 1].stationName === origin && CROSS_CITY.includes(this.routeData.routeName)) startIndex += 1
    if (this.stops[startIndex + 2].stationName === origin && CROSS_CITY.includes(this.routeData.routeName)) startIndex += 2
  ...
  fixDelayJumps() {
    ...
    for (let i = nextStopIndex + 1; i < this.stops.length; i++) {
...
class MetroTripStop {
  ...
  set delay(delay) {
    this.estimatedDeparture = parseISOTime(+this.scheduledDeparture + delay * 1000 * 60)
  }
