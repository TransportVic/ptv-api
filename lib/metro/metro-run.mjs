...
export default class MetroRun extends TransitRun {
  ...
  constructor(runRef, destination, position, description, tdn, isRailBus, viaCityLoop, interchange) {
  ...
  populateInterchange(interchange) {
    ...
    if (interchange.distributor) this.formedBy = MetroRun.getTDN(interchange.distributor.run_ref)
    if (interchange.feeder) this.forming = MetroRun.getTDN(interchange.feeder.run_ref)
  }
  ...
  static fromPTVData(data, flags, line, direction) {
    ...
    return new MetroRun(data.run_ref, data.destination_name, data.vehicle_position, data.vehicle_descriptor, tdn, isRailBus, viaCityLoop, data.interchange)
  }
