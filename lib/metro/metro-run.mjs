...
export default class MetroRun extends TransitRun {
  ...
  constructor(runRef, destination, direction, position, description, tdn, isRailBus, viaCityLoop, interchange, line) {
    let correctedDestination = MetroUtils.correctStationName(destination)
    super(runRef, correctedDestination, direction, position, description)
    this.tdn = tdn
  ...
  populateInterchange(interchange) {
    ...
    if (interchange.feeder && interchange.feeder.run_ref !== '0') this.formedBy = {
      ...
      destination: interchange.feeder.destination_name.trim(),
      advertised: interchange.feeder.advertised
    }
    ...
    if (interchange.distributor && interchange.distributor.run_ref !== '0') this.forming = {
      ...
      destination: interchange.distributor.destination_name.trim(),
      advertised: interchange.distributor.advertised
    }
  }

  /**
   * Checks if a Frankston train runs via the city loop.
   * 
   * @param {string} tdn The TDN of the train
   * @param {string} direction The direction of the train
   * @param {string} destination The train's destination
   * @returns True if the train runs via the city loop
   */
  static handleFKNViaLoop(tdn, direction, destination) {
    return direction === 'Up' && destination === 'Flinders Street'
  }
   ...
   * @returns True if the train runs via the city loop
   */
  static isViaLoop(tdn, routeName, direction, destination) {
    let viaCityLoop
    let loopIndicator = parseInt(tdn[1])

    if (routeName === 'Frankston') {
      return this.handleFKNViaLoop(tdn, direction, destination)
    } else {
  ...
  updateDestination() {
    ...
    else if (lineGroup === 'northern') this.updateDestinationNOR()
  }
  ...
  static fromPTVData(data, flags, line, direction) {
    ...
    let viaCityLoop = (isRailBus || !tdn) ? null : MetroRun.isViaLoop(tdn, line.routeName, direction.railDirection, data.destination_name)

    let run = new MetroRun(data.run_ref, data.destination_name, direction, data.vehicle_position, data.vehicle_descriptor, tdn, isRailBus, viaCityLoop, data.interchange, line)
    if (data.status === 'updated') run.updated = true
    else if (data.status === 'added') run.additional = true
    else if (data.status === 'cancelled') run.cancelled = true
    return run
  }
