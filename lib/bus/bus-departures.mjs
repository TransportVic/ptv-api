...
export default class BusDepartures extends TransitDepartures {
  ...
  async fetch(parameters) {
    let body = await this.fetchDepartures(0, parameters)

    this.responseData = body
    for (let departure of body.departures) {
      this.push(BusDeparture.fromPTVData(this.#api, departure, this.getRoute(departure), this.getRun(departure), this.getDirection(departure)))
    }
  }
}