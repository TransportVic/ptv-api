...
describe('The MetroSiteDeparture class', () => {
  describe('The fetch function', () => {
    it('Should return a list of TDNs with available departure data', async () => {
      ...
      let willamstown = await ptvAPI.metroSite.getDepartures()

      let td1054 = willamstown.find(trip => trip.tdn === '1054')
      ...
      let td4406 = willamstown.find(trip => trip.tdn === '4406')
      ...
      expect(td4406Stops[5].stationName).to.equal('Bonbeach')
      expect(td4406Stops[5].platform).to.equal('1')
      expect(dateLikeToISO(td4406Stops[0].scheduledDeparture)).to.equal('2025-04-20T05:49:00.000Z')
      expect(dateLikeToISO(td4406Stops[0].estimatedArrival)).to.equal('2025-04-20T05:50:00.000Z')
      expect(dateLikeToISO(td4406Stops[0].estimatedDeparture)).to.equal('2025-04-20T05:51:00.000Z')
    })
  })
})